...
from onnx.reference.ops._op_list import Split_13, Unsqueeze_1, Slice_10
...
class ONNXParser:
    ...
    def maxpoolEquations(self, node, makeEquations):
        ...
        is_conv1d = (len(inputShape) == 3) or (len(inputShape) == 4 and inputShape[3] == 1)
        
        if is_conv1d:
        # Conv1D-like 처리
            input_channels = inputShape[1]
            input_width = inputShape[2]
            output_width = int(np.ceil((input_width - kernel_shape[0] + 1) / strides[0]))
            outputShape = [inputShape[0], input_channels, output_width]
        ...
        if is_conv1d:
            for i in range(outputShape[2]):
                for c in range(outputShape[1]):
                    maxVars = set()
                    for di in range(strides[0]*i, strides[0]*i + kernel_shape[0]):
                        if di < input_width:
                            var = inVars[0][c][di]
                            if isinstance(var, np.ndarray):  # numpy array라면
                                var = var.item()  # scalar로 변환
                            maxVars.add(var)
                            # maxVars.add(inVars[0][c][di])
                    self.query.addMaxConstraint(maxVars, outVars[0][c][i])
        else:
            for i in range(outputShape[2]):
    ...
    def convEquations(self, node, makeEquations):
        ...
        if len(pads) == 2:
            pad_left = pad_right = pads[0]
            pad_top = pad_bottom = pads[1]
        elif len(pads) == 4:
            pad_left, pad_top, pad_right, pad_bottom = pads
        else:
            raise ValueError(f"Unsupported number of pads: {pads}")
        ...
        if (len(shape0) == 3 and len(shape1) == 3) or (len(shape0) == 4 and shape0[3] == 1 and len(shape1) == 3):
            # Conv1D 처리: [N, C, L]
            input_channels = shape0[1]
            input_width = shape0[2]
            input_height = 1  # dummy
            num_filters = shape1[0]
            filter_channels = shape1[1]
            filter_width = shape1[2]
            filter_height = 1  # dummy
        elif len(shape0) == 4 and len(shape1) == 4:
            # Conv2D 처리: [N, C, H, W]
            input_channels = shape0[1]
            input_height = shape0[2]
            input_width = shape0[3]
            num_filters = shape1[0]
            filter_channels = shape1[1]
            filter_height = shape1[2]
            filter_width = shape1[3]
        else:
            raise ValueError(f"Unsupported input shape for Conv: {shape0}")
        ...
        if (len(shape0) == 3 and len(shape1) == 3) or (len(shape0) == 4 and shape0[3] == 1 and len(shape1) == 3): 
            ...
            raise ValueError(f"Unsupported input shape: {shape0}")
    def gemmEquations(self, node, makeEquations):
        ...
        assert shape1[-1] == shape2[0], f"Incompatible shapes: {shape1} and {shape2}"
    ...
    def splitEquations(self, node, nodeName, makeEquations):
        ...
        inputName = node.input[0]
        inputVars = Split_13.eval(self.varMap[inputName], split=split, axis=axis)
